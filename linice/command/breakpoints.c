...
void DisarmBreakpoints(void)
{
    ...
    int index;
    int eBPX = -1;                      // Index of a BPX that matches cs:eip-1
    ...
    // Print the breakpoint message
    if( eBPX != -1 )
    {
        dprinth(1, "Breakpoint due to BPX %02X", eBPX);
    }
    ...
    if( (eBPX != -1) && (deb.sysReg.dr6 & (DR6_BS_BIT|DR6_BT_BIT|0xF))==0 )
    {
        deb.r->eip -= 1;
    }
